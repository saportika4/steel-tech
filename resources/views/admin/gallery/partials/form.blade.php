...
<form id="{{ $isEdit ? 'editGalleryForm' : 'createGalleryForm' }}" enctype="multipart/form-data">
    @csrf
    @if($isEdit)
        @method('PUT')
    @endif
        ...
        <div>
            ...
            <label class="w-12 h-6 relative block mt-2">
                <input
                    ...
                    {{ old('is_active', isset($gallery) ? (int)$gallery->is_active : 1) ? 'checked' : '' }}
                >
                <span class="bg-[#ebedf2] dark:bg-dark block h-full rounded-full before:absolute before:left-1 before:bg-white dark:before:bg-white-dark dark:peer-checked:before:bg-white before:bottom-1 before:w-4 before:h-4 before:rounded-full peer-checked:before:left-7 peer-checked:bg-primary before:transition-all before:duration-300"></span>
            </label>
            <small class="text-white-dark">Toggle to activate/deactivate image</small>
        </div>
    </div>
    ...
    <div class="mt-5">
        ...
        <textarea
            ...
            rows="4"
            class="form-textarea"
            placeholder="Enter a short description or caption for this image"
    ...
    <div class="mt-5">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary transition-colors">
            ...
            <label for="galleryImage" class="cursor-pointer">
                <div id="imagePreviewContainer" class="mb-3">
                    @if($isEdit && $gallery->image_url)
                        <img id="imagePreview" src="{{ $gallery->image_url }}" alt="Preview" class="max-w-xs mx-auto rounded-lg shadow-lg">
                    @else
                ...
                <p class="text-xs text-gray-500 mt-1">PNG, JPG, WEBP up to 5 MB</p>
            </label>
        </div>
    ...
    <div class="flex justify-end items-center mt-8 gap-4">
        <a href="{{ route('admin.gallery.index') }}" class="btn btn-outline-danger">Cancel</a>
